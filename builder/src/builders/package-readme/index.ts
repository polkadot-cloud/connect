...
export const generatePackageReadme = async (pkg: string) => {
	try {
		const readmePath = `${getPackageDirectory(pkg)}/README.md`
		...
		try {
			...
			if (
				existingReadme &&
				!existingReadme.includes('Part of the [polkadot-cloud/connect]')
			) {
...
export const getPackageData = async (name: string): Promise<PackageData> => {
	...
	try {
		packageJson = await getPackageJson(getPackageDirectory(name))
	} catch (e) {
	...
	const displayName =
		...
		packageJson.name?.replace(/-source$/, '').replace('@polkadot-cloud/', '') ||
		name
